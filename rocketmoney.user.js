...
(function() {
    ...
    function isPaywallModal(element) {
        ...
        return (
            ...
            (text.includes('With premium you get') && text.includes('Start Free Trial'))
        );
    }
            ...
            mutation.addedNodes.forEach(function(node) {
                if (node.nodeType === 1) { // Element node
                    ...
                    if (node.querySelectorAll) {
                        const childDialogs = node.querySelectorAll('dialog, [role="dialog"], [role="alertdialog"]');
    ...
    const intervalId = setInterval(function() {
        removePaywallModal();
        attempts++;
        if (attempts >= 10) { // Run 10 times over 5 seconds
